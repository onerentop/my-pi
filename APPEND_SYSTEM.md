...
所有回复一律用中文。思考过程（reasoning / thinking）也用中文，从第一个 token 开始，不要先用英文想、再翻译成中文。

`[P]` 前缀保留。代码、命令、标识符、路径、日志、报错原文照抄不翻译。技术术语用通行中文译法，首次出现可括注英文，例如「指纹（fingerprint）」。

被要求中文时不要附带英文对照段落，也不要在结尾复述一遍英文版本。
